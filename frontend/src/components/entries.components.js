...
const Entries =() =>{
        ...
        {entries != null && entries.map((entry, i) =>(
            ...
            <Modal.Body>
                <Form.Group>
                    ...
                    <Form.Control type="number" onChange={(event) => {newEntry.calories = event.target.valueAsNumber}}></Form.Control>
                    <Form.Label>fat</Form.Label>
                    <Form.Control type="number" onChange={(event) => {newEntry.fat = event.target.valueAsNumber}}></Form.Control>
                </Form.Group>
    ...
    function changeIngredientForEntry(){
        ...
        var url = "http://localhost:3001/ingredient/update?id=" + changeIngredient.id
        axios.put(url, {
    ...
    function changeSingleEntry(){
        ...
        var url = "http://localhost:3001/entry/update?id=" + changeEntry.id
    ...
    function addSingleEntry(){
        ...
        var url = "http://localhost:3001/entry/add"
        axios.post(url, {
    ...
    function deleteSingleEntry(id){
        var url = "http://localhost:3001/entry/delete?id=" + id
        axios.delete(url, {
    ...
    function getAllEntries(){
        var url = "http://localhost:3001/entries"
        axios.get(url, {
            reponseType: 'json'
        }).then(response => {
            if(response.status == 200){
